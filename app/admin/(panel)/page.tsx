...
export default async function AdminDashboard() {
  ...
  const [dir, sank, votes, shakthi] = await Promise.all([
    ...
    supabase.from("shakthi_registrations").select("status").limit(2000),
  ]);
  ...
  const shakthiRows = (shakthi.data ?? []) as { status: string }[];
  const shakthiConfirmed = shakthiRows.filter((r) => r.status === "confirmed").length;
  ...
  return (
    <>
      ...
      <h2 style={{ fontSize: "1.05rem", margin: "28px 0 12px" }}>
        <Link href="/admin/shakthi">Shakthi Nadappu walk</Link>
      </h2>
      <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
